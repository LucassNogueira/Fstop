    ...
    <section className="text-gray-600 body-font py-10 border-y-2 border-indigo-600">
      <div className="container mx-auto flex px-0 py-0 md:flex-row flex-col items-center">
        <div className="lg:max-w-xl lg:w-full lg:h-full md:w-full w-full mb-10 md:mb-0">
          <img
            className="mx-auto"
            alt={userDoc?.favTeam[0]?.name}
            src={userDoc?.favTeam[0]?.logo}
          />
        </div>
        <div className=" md:w-1/2  flex flex-col md:items-start md:text-left items-center text-center">
          <h1 className="title-font sm:text-4xl text-3xl mb-4 font-medium text-gray-900">
            {userDoc?.favTeam[0].name}
          </h1>
          <p className="mb-8 leading-relaxed">
            {userDoc?.favTeam[0].name} has accumulated{" "}
            {userDoc?.favTeam[0].fastest_laps} fastest laps, with a PSU built by{" "}
            {userDoc?.favTeam[0].engine}. Since the teams entry into Formula 1
            in {userDoc?.favTeam[0].first_team_entry}, the team has held pole
